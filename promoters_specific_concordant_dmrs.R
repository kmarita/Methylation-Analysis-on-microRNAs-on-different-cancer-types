...
cat(sprintf(" Loaded %d miRNA gene coordinates from BED file\n", nrow(mirna_genes)))

read_all_miRNA_dmrs <- function(cancer_type, results_dir, mirna_genes) {
  file_path <- file.path(results_dir, cancer_type, 
                         paste0(cancer_type, "_DMRs_in_miRNA_promoters.tsv"))
  ...
  all_dmrs <- read.table(file_path, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  
  required_cols <- c("Chromosome", "Start", "End")
  missing_cols <- setdiff(required_cols, colnames(all_dmrs))
  ...
  if (!"DMR_ID" %in% colnames(all_dmrs)) {
    all_dmrs$DMR_ID <- paste0("DMR_", 1:nrow(all_dmrs))
  }
  
  all_dmrs <- all_dmrs[!is.na(all_dmrs$Chromosome) & 
                         !is.na(all_dmrs$Start) & 
                         !is.na(all_dmrs$End), ]
  
  all_dmrs <- all_dmrs[all_dmrs$End >= all_dmrs$Start, ]
  all_dmrs <- all_dmrs[all_dmrs$Start >= 0 & all_dmrs$End >= 0, ]
  ...
  # Use existing miRNA column if it exists, otherwise assign from BED
  if (!"miRNA" %in% colnames(all_dmrs)) {
    all_dmrs$miRNA <- NA_character_
    ...
    for (i in 1:nrow(mirna_genes)) {
      ...
      overlaps <- all_dmrs$Chromosome == chr & 
        all_dmrs$Start < end & 
        all_dmrs$End > start
      
      all_dmrs$miRNA[overlaps] <- mirna_name
    }
    
    all_dmrs <- all_dmrs[!is.na(all_dmrs$miRNA), ]
  }
  
  cat(sprintf(" Loaded %d total DMRs in miRNA Promoter  from %s\n", nrow(all_dmrs), cancer_type))
  return(all_dmrs)
}

read_miRNA_dmrs <- function(cancer_type, results_dir, mirna_genes) {
  file_path <- file.path(results_dir, cancer_type, 
                         paste0(cancer_type, "_DMRs_in_miRNA_promoters.tsv"))
  ...
  if (!"DMR_Status" %in% colnames(dmrs)) {
    cat(sprintf("Column 'DMR_Status' not found in %s\n", cancer_type))
    cat("Available columns:\n")
    print(colnames(dmrs))
    return(NULL)
  }
  
  dmrs_sig <- dmrs[dmrs$DMR_Status %in% c("Hypermethylated", "Hypomethylated"), ]
  
  if (nrow(dmrs_sig) == 0) {
    cat(sprintf("No significant DMRs for %s\n", cancer_type))
    cat("Unique values in DMR_Status:\n")
    print(unique(dmrs$DMR_Status))
    return(NULL)
  }
  
  dmrs_sig$DMR_Status <- dmrs_sig$DMR_Status
  
  required_cols <- c("Chromosome", "Start", "End")
  missing_cols <- setdiff(required_cols, colnames(dmrs_sig))
  ...
  if (!"DMR_ID" %in% colnames(dmrs_sig)) {
    dmrs_sig$DMR_ID <- paste0("DMR_", 1:nrow(dmrs_sig))
  }
  
  dmrs_sig <- dmrs_sig[!is.na(dmrs_sig$Chromosome) & 
                         !is.na(dmrs_sig$Start) & 
                         !is.na(dmrs_sig$End), ]
  
  dmrs_sig <- dmrs_sig[dmrs_sig$End >= dmrs_sig$Start, ]
  dmrs_sig <- dmrs_sig[dmrs_sig$Start >= 0 & dmrs_sig$End >= 0, ]
  
  if (nrow(dmrs_sig) == 0) {
    cat(sprintf("No valid DMRs remaining for %s after QC\n", cancer_type))
    return(NULL)
  }
  
  # Use existing miRNA column if it exists, otherwise assign from BED
  if (!"miRNA" %in% colnames(dmrs_sig)) {
    dmrs_sig$miRNA <- NA_character_
    ...
    for (i in 1:nrow(mirna_genes)) {
      ...
      overlaps <- dmrs_sig$Chromosome == chr & 
        dmrs_sig$Start < end & 
        dmrs_sig$End > start
      
      dmrs_sig$miRNA[overlaps] <- mirna_name
    }
    
    dmrs_sig <- dmrs_sig[!is.na(dmrs_sig$miRNA), ]
  }
...
all_dmr_list <- list()
all_dmrs_sig_list <- list()

for (cancer in cancer_types) {
  all_dmrs <- read_all_miRNA_dmrs(cancer, results_dir, mirna_genes)
  if (!is.null(all_dmrs)) {
    all_dmr_list[[cancer]] <- all_dmrs
  }
  
  dmrs_sig <- read_miRNA_dmrs(cancer, results_dir, mirna_genes)
  if (!is.null(dmrs_sig)) {
    all_dmrs_sig_list[[cancer]] <- dmrs_sig
  }
}

if (length(all_dmrs_sig_list) == 0) {
  stop("No miRNA DMR files found with significant results.")
}

cat(sprintf("Total cancer types with DMR data: %d\n", length(all_dmr_list)))
cat(sprintf("Total cancer types with significant miRNA DMRs: %d\n", length(all_dmrs_sig_list)))


dmr_counts_per_mirna_cancer <- data.frame()

for (cancer in names(all_dmr_list)) {
  dmr_data <- all_dmr_list[[cancer]]
  
  mirna_counts <- dmr_data %>%
    group_by(miRNA) %>%
    summarise(Total_DMRs = n(), .groups = "drop") %>%
    mutate(Cancer = cancer)
  
  dmr_counts_per_mirna_cancer <- rbind(dmr_counts_per_mirna_cancer, mirna_counts)
}

cat(sprintf("Calculated DMR counts for %d miRNA-cancer combinations\n", 
            nrow(dmr_counts_per_mirna_cancer)))

dmr_counts_summary <- dmr_counts_per_mirna_cancer %>%
  group_by(miRNA) %>%
  summarise(
...
cat("\nDMR count summary (top 10 miRNAs by mean DMRs):\n")
print(head(dmr_counts_summary[order(-dmr_counts_summary$Mean_DMRs), ], 10))

dmr_counts_file <- file.path(results_dir, "miRNA_promoter_DMR_counts_per_cancer.tsv")
write.table(dmr_counts_per_mirna_cancer, file = dmr_counts_file, 
            sep = "\t", quote = FALSE, row.names = FALSE)
cat(sprintf("\n✓ Saved DMR counts: %s\n", dmr_counts_file))

all_dmrs <- data.frame()

cancer_names <- names(all_dmrs_sig_list)
for (cancer in cancer_names) {
  dmrs_sig <- all_dmrs_sig_list[[cancer]]
  
  if (nrow(dmrs_sig) > 0) {
    dmr_info <- dmrs_sig[, c("DMR_ID", "Chromosome", "Start", "End", "miRNA", "DMR_Status")]
...
cat("Formula: [Sum(Significant DMRs in all miRNAs)] / [Sum(Total DMRs in all miRNAs)] per cancer\n\n")

dmr_sig_counts_per_mirna_cancer <- all_dmrs %>%
  group_by(Cancer, miRNA, DMR_Status) %>%
  summarise(DMR_Count = n(), .groups = "drop")

total_dmrs_sig_per_mirna_cancer <- dmr_sig_counts_per_mirna_cancer %>%
  group_by(Cancer, miRNA) %>%
  summarise(Total_Sig_DMRs = sum(DMR_Count), .groups = "drop")

mirna_dmr_counts <- total_dmrs_sig_per_mirna_cancer %>%
  left_join(dmr_counts_per_mirna_cancer, by = c("Cancer", "miRNA"))

mirna_dmr_counts <- mirna_dmr_counts %>%
  filter(!is.na(Total_DMRs))

if (nrow(mirna_dmr_counts) == 0) {
  stop("No matching miRNA names between significant DMR data and total DMR counts!")
}

cat(sprintf("Successfully matched %d miRNA-cancer pairs with DMR counts\n", 
            nrow(mirna_dmr_counts)))

normalization_per_cancer <- mirna_dmr_counts %>%
  group_by(Cancer) %>%
  summarise(
...
dmr_status_with_total <- dmr_sig_counts_per_mirna_cancer %>%
  left_join(dmr_counts_per_mirna_cancer, by = c("Cancer", "miRNA")) %>%
  filter(!is.na(Total_DMRs))

dmr_status_sums <- dmr_status_with_total %>%
  group_by(Cancer, DMR_Status) %>%
  summarise(
    Sum_Sig_DMRs_Status = sum(DMR_Count),
    Sum_Total_DMRs = sum(Total_DMRs),
    .groups = "drop"
  )
...
normalization_stats_file <- file.path(results_dir, "dmr_normalization_statistics.tsv")
...
p_status_norm <- ggplot(dmr_status_sums, aes(x = Cancer, y = Normalized_Ratio, fill = DMR_Status)) +
  ...
  labs(
    title = "Hypermethylated vs Hypomethylated miRNA Promoter  DMRs per Cancer Type",
...
plot_status_norm_file <- file.path(results_dir, "dmr_status_hyper_hypo_promoter_per_cancer_NORMALIZED.pdf")
...
cat(sprintf("\nAnalyzing concordance across %d unique DMR positions...\n", length(dmr_coords)))
...
for (coord in dmr_coords) {
  ...
  if (num_cancers >= 2) {
    ...
    dmr_concordance <- rbind(dmr_concordance, data.frame(
      Coord = coord,
      DMR_ID = paste(unique(coord_data$DMR_ID), collapse = ";"),
...
cat(sprintf("\nTotal Promoter  DMR positions in 2+ cancers: %d\n", nrow(dmr_concordance)))
cat(sprintf("Concordant Promoter  DMRs (all same direction): %d\n", nrow(concordant_dmrs)))
...
cat(sprintf("Discordant Promoter  DMRs (mixed directions): %d\n\n", sum(dmr_concordance$Concordance == "Discordant")))

cat("Distribution of Promoter  DMRs by number of cancers:\n")
...
if (nrow(concordant_dmrs) > 0) {
  ...
  for (mirna in rownames(dmr_matrix_norm)) {
    for (cancer in colnames(dmr_matrix_norm)) {
      dmr_count <- dmr_counts_per_mirna_cancer %>%
        filter(miRNA == mirna, Cancer == cancer) %>%
        pull(Total_DMRs)
      
      if (length(dmr_count) > 0 && dmr_count > 0) {
        dmr_matrix_norm[mirna, cancer] <- (dmr_matrix[mirna, cancer] / dmr_count) * 100
      } else {
        dmr_matrix_norm[mirna, cancer] <- 0
      }
    }
  }
  ...
  dmr_matrix_norm_df <- cbind(miRNA = rownames(dmr_matrix_norm_df), dmr_matrix_norm_df)
  rownames(dmr_matrix_norm_df) <- NULL
  
  output_matrix_file <- file.path(results_dir, "miRNA_promoter_cancer_dmr_matrix.tsv")
  write.table(dmr_matrix_df, file = output_matrix_file, sep = "\t", quote = FALSE, row.names = FALSE)
  
  output_matrix_norm_file <- file.path(results_dir, "miRNA_promoter_cancer_dmr_matrix_NORMALIZED.tsv")
  write.table(dmr_matrix_norm_df, file = output_matrix_norm_file, sep = "\t", quote = FALSE, row.names = FALSE)
  
  cat(sprintf("\nmiRNA Promoter  x Cancer DMR matrix created with %d miRNAs and %d cancer types\n", 
  ...
  cat("\nTop 20 miRNAs by total Promoter  DMRs across all cancers:\n")
  print(top_mirnas)
  ...
  output_file1 <- file.path(results_dir, "concordant_miRNA_promoter_DMRs_summary.tsv")
  write.table(miRNA_summary, file = output_file1, sep = "\t", quote = FALSE, row.names = FALSE)
  
  output_file2 <- file.path(results_dir, "concordant_promoter_DMRs_detailed.tsv")
  write.table(concordant_dmrs, file = output_file2, sep = "\t", quote = FALSE, row.names = FALSE)
  
YTG  cat(sprintf("miRNA Promoter -Cancer DMR matrix: %s\n", output_matrix_file))
  cat(sprintf("miRNA Promoter -Cancer NORMALIZED DMR matrix: %s\n", output_matrix_norm_file))
  cat(sprintf("Concordant miRNA Promoter  DMRs summary: %s\n", output_file1))
  cat(sprintf("Concordant Promoter  DMRs detailed: %s\n", output_file2))
  ...
  p2 <- ggplot(cancer_count_df, aes(x = Num_Cancers, y = Normalized_Percent)) +
    ...
    geom_text(aes(label = sprintf("%.1f%%", Normalized_Percent)), 
              vjust = -0.5, size = 3.5) +
    theme_minimal(base_size = 12) +
    labs(title = "Distribution of Promoter  DMRs by Number of Cancers",
         x = "Number of Cancers",
         y = "Percentage of Total Concordant DMRs (%)") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
    ylim(0, max(cancer_count_df$Normalized_Percent) * 1.15)
  
  plot2_file <- file.path(results_dir, "promoter_dmr_cancer_distribution_NORMALIZED.pdf")
  ...
  for (mirna in top20_summary$miRNA) {
    dmr_data <- dmr_counts_per_mirna_cancer %>%
      filter(miRNA == mirna)
    
    if (nrow(dmr_data) > 0) {
      top20_dmr_summary <- rbind(top20_dmr_summary, data.frame(
        miRNA = mirna,
        Mean_DMRs = mean(dmr_data$Total_DMRs),
        stringsAsFactors = FALSE
      ))
    }
  }
  ...
  top20_summary <- top20_summary %>%
    left_join(top20_dmr_summary, by = "miRNA")
  
  top20_summary$Hyper_Percent <- (top20_summary$All_Hypermethylated_DMRs / top20_summary$Mean_DMRs) * 100
  top20_summary$Hypo_Percent <- (top20_summary$All_Hypomethylated_DMRs / top20_summary$Mean_DMRs) * 100
  
  top20_long <- top20_summary %>%
    select(miRNA, Hyper_Percent, Hypo_Percent, Mean_DMRs, Total_Concordant_DMRs) %>%
    pivot_longer(
  ...
  p3 <- ggplot(top20_long, aes(x = miRNA, y = Percent, fill = DMR_Status)) +
    ...
    theme_minimal(base_size = 12) +
    labs(
      title = "Top 20 miRNAs by Concordant Promoter  DMR Count (Normalized by Total DMRs)",
  ...
  plot3_file <- file.path(results_dir, "top20_mirnas_promoter_dmrs_NORMALIZED.pdf")
  ...
  if (nrow(chr_df) > 0) {
    ...
    p4 <- ggplot(chr_df, aes(x = Chromosome, y = Normalized_Percent)) +
      ...
      labs(title = "Concordant Promoter  DMRs Distribution Across Chromosomes",
           x = "Chromosome",
    ...
    plot4_file <- file.path(results_dir, "promoter_dmrs_by_chromosome_NORMALIZED.pdf")
  ...
  p5 <- ggplot(cancer_norm_data, aes(x = Cancer, y = Normalized_Percent)) +
    ...
    theme_minimal(base_size = 11) +
    labs(
      title = "Concordant Promoter  DMRs per Cancer Type (Normalized by Total DMRs)",
  ...
  plot5_file <- file.path(results_dir, "promoter_dmrs_per_cancer_NORMALIZED.pdf")
  ...
  if (exists("dmr_matrix_norm")) {
    ...
    multi_cancer_mirnas <- rowSums(dmr_matrix_norm > 0) >= min_cancer_types
    cat(sprintf("miRNAs present in >= %d cancers: %d\n", 
                min_cancer_types, sum(multi_cancer_mirnas)))
    
    dmr_matrix_filtered <- dmr_matrix_norm[multi_cancer_mirnas, , drop = FALSE]
  ...
  if (nrow(dmr_matrix_filtered) > top_n_mirnas) {
    mirna_sums <- rowSums(dmr_matrix_filtered)
    top_mirnas_idx <- order(mirna_sums, decreasing=TRUE)[1:top_n_mirnas]
    dmr_matrix_top <- dmr_matrix_filtered[top_mirnas_idx, , drop=FALSE]
    cat(sprintf("Selected top %d miRNAs by normalized DMR percentage\n", top_n_mirnas))
  } else {
  ...
  if (nrow(dmr_matrix_top) == 0 || ncol(dmr_matrix_top) == 0) {
    ...
    cat(sprintf("Value range: %.2f%% to %.2f%%\n", min(dmr_matrix_top), max(dmr_matrix_top)))
    cat("\nMatrix preview (first 5 rows, first 5 cols):\n")
    print(dmr_matrix_top[1:min(5, nrow(dmr_matrix_top)), 
                         1:min(5, ncol(dmr_matrix_top))])
    
    plot6_file <- file.path(results_dir, "miRNA_promoter_cancer_heatmap_DMR_NORMALIZED.pdf")
    ...
    tryCatch({
      ...
      pheatmap(
        ...
        fontsize_row = 8,
        fontsize_col = 10,
        main = paste0("Normalized Concordant Promoter  DMRs: Top ", nrow(dmr_matrix_top),
                      " miRNAs\n(Significant DMR / Total DMR × 100%)"),
        border_color = "grey60"
      )
      
      dev.off()
      cat(sprintf(" Saved Plot 6 (Heatmap - normalized by total DMRs): %s\n", plot6_file))
  ...
  cat("No concordant Promoter  DMRs found.\n")
}

cat("\n=== ALL PLOTS GENERATED (6 TOTAL - ALL NORMALIZED BY TOTAL DMR COUNT) ===\n")
cat("1. dmr_status_hyper_hypo_promoter _per_cancer_NORMALIZED.pdf\n")
cat("2. promoter_dmr_cancer_distribution_NORMALIZED.pdf\n")
cat("3. top20_mirnas_promoter_dmrs_NORMALIZED.pdf\n")
cat("4. promoter_dmrs_by_chromosome_NORMALIZED.pdf\n")
cat("5. promoter_dmrs_per_cancer_NORMALIZED.pdf\n")
cat("6. miRNA_promoter_cancer_heatmap_DMR_NORMALIZED.pdf\n")
