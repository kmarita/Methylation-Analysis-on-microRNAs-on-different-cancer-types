library(dplyr)
library(ggplot2)
library(pheatmap)
library(RColorBrewer)
library(tidyr)

main_dir <- "~/methylation_project"
results_dir <- file.path(main_dir, "methylation_analysis_results")

cancer_types <- c("TCGA-BRCA","TCGA-BLCA", "TCGA-CESC", "TCGA-CHOL", "TCGA-COAD", 
                  "TCGA-ESCA", "TCGA-GBM", "TCGA-HNSC", "TCGA-KIRC", "TCGA-KIRP",
                  "TCGA-LIHC","TCGA-LUSC", "TCGA-LUAD", "TCGA-PAAD", "TCGA-PCPG", 
                  "TCGA-PRAD", "TCGA-READ", "TCGA-SARC", "TCGA-SKCM", "TCGA-STAD", 
                  "TCGA-THYM", "TCGA-UCEC")

mirna_bed_file <- file.path(main_dir, "miRNA_genes.hg19.sorted.bed")

mirna_genes <- read.table(mirna_bed_file, header = FALSE, sep = "\t", 
                          stringsAsFactors = FALSE, comment.char = "")
colnames(mirna_genes) <- c("Chromosome", "Start", "End", "miRNA", "Score", "Strand")

cat(sprintf("Loaded %d miRNA gene coordinates from BED file\n", nrow(mirna_genes)))

library(dplyr)
library(ggplot2)
library(pheatmap)
library(RColorBrewer)
library(tidyr)

main_dir <- "~/methylation_project"
results_dir <- file.path(main_dir, "methylation_analysis_results")

cancer_types <- c("TCGA-BRCA","TCGA-BLCA", "TCGA-CESC", "TCGA-CHOL", "TCGA-COAD", 
                  "TCGA-ESCA", "TCGA-GBM", "TCGA-HNSC", "TCGA-KIRC", "TCGA-KIRP",
                  "TCGA-LIHC","TCGA-LUSC", "TCGA-LUAD", "TCGA-PAAD", "TCGA-PCPG", 
                  "TCGA-PRAD", "TCGA-READ", "TCGA-SARC", "TCGA-SKCM", "TCGA-STAD", 
                  "TCGA-THYM", "TCGA-UCEC")

mirna_bed_file <- file.path(main_dir, "miRNA_genes.hg19.sorted.bed")

mirna_genes <- read.table(mirna_bed_file, header = FALSE, sep = "\t", 
                          stringsAsFactors = FALSE, comment.char = "")
colnames(mirna_genes) <- c("Chromosome", "Start", "End", "miRNA", "Score", "Strand")

cat(sprintf("Loaded %d miRNA gene coordinates from BED file\n", nrow(mirna_genes)))

read_all_miRNA_dmrs <- function(cancer_type, results_dir, mirna_genes) {
  file_path <- file.path(results_dir, cancer_type, 
                         paste0(cancer_type, "_DMRs_in_miRNA.tsv"))
  
  if (!file.exists(file_path)) {
    cat(sprintf("File not found for %s\n", cancer_type))
    return(NULL)
  }
  
  all_dmrs <- read.table(file_path, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  
  required_cols <- c("Chromosome", "Start", "End")
  missing_cols <- setdiff(required_cols, colnames(all_dmrs))
  
  if (length(missing_cols) > 0) {
    cat(sprintf("Missing columns in %s: %s\n", cancer_type, paste(missing_cols, collapse=", ")))
    return(NULL)
  }
  
  if (!"DMR_ID" %in% colnames(all_dmrs)) {
    all_dmrs$DMR_ID <- paste0("DMR_", 1:nrow(all_dmrs))
  }
  
  all_dmrs <- all_dmrs[!is.na(all_dmrs$Chromosome) & 
                         !is.na(all_dmrs$Start) & 
                         !is.na(all_dmrs$End), ]
  
  all_dmrs <- all_dmrs[all_dmrs$End >= all_dmrs$Start, ]
  all_dmrs <- all_dmrs[all_dmrs$Start >= 0 & all_dmrs$End >= 0, ]
  
  if (nrow(all_dmrs) == 0) {
    cat(sprintf("No valid DMRs for %s after QC\n", cancer_type))
    return(NULL)
  }
  
  if (!"miRNA" %in% colnames(all_dmrs)) {
    all_dmrs$miRNA <- NA_character_
    
    for (i in 1:nrow(mirna_genes)) {
      chr <- mirna_genes$Chromosome[i]
      start <- mirna_genes$Start[i]
      end <- mirna_genes$End[i]
      mirna_name <- mirna_genes$miRNA[i]
      
      overlaps <- all_dmrs$Chromosome == chr & 
        all_dmrs$Start < end & 
        all_dmrs$End > start
      
      all_dmrs$miRNA[overlaps] <- mirna_name
    }
    
    all_dmrs <- all_dmrs[!is.na(all_dmrs$miRNA), ]
  }
  
  cat(sprintf("Loaded %d total DMRs in miRNA gene bodies from %s\n", nrow(all_dmrs), cancer_type))
  return(all_dmrs)
}

read_miRNA_dmrs <- function(cancer_type, results_dir, mirna_genes) {
  file_path <- file.path(results_dir, cancer_type, 
                         paste0(cancer_type, "_DMRs_in_miRNA.tsv"))
  
  if (!file.exists(file_path)) {
    cat(sprintf("File not found for %s\n", cancer_type))
    return(NULL)
  }
  
  dmrs <- read.table(file_path, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  
  if (!"DMR_Status" %in% colnames(dmrs)) {
    cat(sprintf("Column 'DMR_Status' not found in %s\n", cancer_type))
    cat("Available columns:\n")
    print(colnames(dmrs))
    return(NULL)
  }
  
  dmrs_sig <- dmrs[dmrs$DMR_Status %in% c("Hypermethylated", "Hypomethylated"), ]
  
  if (nrow(dmrs_sig) == 0) {
    cat(sprintf("No significant DMRs for %s\n", cancer_type))
    cat("Unique values in DMR_Status:\n")
    print(unique(dmrs$DMR_Status))
    return(NULL)
  }
  
  dmrs_sig$DMR_Status <- dmrs_sig$DMR_Status
  
  required_cols <- c("Chromosome", "Start", "End")
  missing_cols <- setdiff(required_cols, colnames(dmrs_sig))
  
  if (length(missing_cols) > 0) {
    cat(sprintf("Missing columns in %s: %s\n", cancer_type, paste(missing_cols, collapse=", ")))
    return(NULL)
  }
  
  if (!"DMR_ID" %in% colnames(dmrs_sig)) {
    dmrs_sig$DMR_ID <- paste0("DMR_", 1:nrow(dmrs_sig))
  }
  
  dmrs_sig <- dmrs_sig[!is.na(dmrs_sig$Chromosome) & 
                         !is.na(dmrs_sig$Start) & 
                         !is.na(dmrs_sig$End), ]
  
  dmrs_sig <- dmrs_sig[dmrs_sig$End >= dmrs_sig$Start, ]
  dmrs_sig <- dmrs_sig[dmrs_sig$Start >= 0 & dmrs_sig$End >= 0, ]
  
  if (nrow(dmrs_sig) == 0) {
    cat(sprintf("No valid DMRs remaining for %s after QC\n", cancer_type))
    return(NULL)
  }
  
  # Use existing miRNA column if it exists, otherwise assign from BED
  if (!"miRNA" %in% colnames(dmrs_sig)) {
    dmrs_sig$miRNA <- NA_character_
    
    for (i in 1:nrow(mirna_genes)) {
      chr <- mirna_genes$Chromosome[i]
      start <- mirna_genes$Start[i]
      end <- mirna_genes$End[i]
      mirna_name <- mirna_genes$miRNA[i]
      
      overlaps <- dmrs_sig$Chromosome == chr & 
        dmrs_sig$Start < end & 
        dmrs_sig$End > start
      
      dmrs_sig$miRNA[overlaps] <- mirna_name
    }
    
    dmrs_sig <- dmrs_sig[!is.na(dmrs_sig$miRNA), ]
  }
  
  cat(sprintf("Loaded %d significant miRNA DMRs from %s\n", nrow(dmrs_sig), cancer_type))
  return(dmrs_sig)
}


all_dmr_list <- list()
all_dmrs_sig_list <- list()

for (cancer in cancer_types) {
  all_dmrs <- read_all_miRNA_dmrs(cancer, results_dir, mirna_genes)
  if (!is.null(all_dmrs)) {
    all_dmr_list[[cancer]] <- all_dmrs
  }
  
  dmrs_sig <- read_miRNA_dmrs(cancer, results_dir, mirna_genes)
  if (!is.null(dmrs_sig)) {
    all_dmrs_sig_list[[cancer]] <- dmrs_sig
  }
}

if (length(all_dmrs_sig_list) == 0) {
  stop("No miRNA DMR files found with significant results.")
}

cat(sprintf("Total cancer types with DMR data: %d\n", length(all_dmr_list)))
cat(sprintf("Total cancer types with significant miRNA DMRs: %d\n", length(all_dmrs_sig_list)))



dmr_counts_per_mirna_cancer <- data.frame()

for (cancer in names(all_dmr_list)) {
  dmr_data <- all_dmr_list[[cancer]]
  
  mirna_counts <- dmr_data %>%
    group_by(miRNA) %>%
    summarise(Total_DMRs = n(), .groups = "drop") %>%
    mutate(Cancer = cancer)
  
  dmr_counts_per_mirna_cancer <- rbind(dmr_counts_per_mirna_cancer, mirna_counts)
}

cat(sprintf("✓ Calculated DMR counts for %d miRNA-cancer combinations\n", 
            nrow(dmr_counts_per_mirna_cancer)))

dmr_counts_summary <- dmr_counts_per_mirna_cancer %>%
  group_by(miRNA) %>%
  summarise(
    Mean_DMRs = mean(Total_DMRs),
    Min_DMRs = min(Total_DMRs),
    Max_DMRs = max(Total_DMRs),
    Num_Cancers = n(),
    .groups = "drop"
  )

cat("\nDMR count summary (top 10 miRNAs by mean DMRs):\n")
print(head(dmr_counts_summary[order(-dmr_counts_summary$Mean_DMRs), ], 10))

dmr_counts_file <- file.path(results_dir, "miRNA_DMR_counts_per_cancer.tsv")
write.table(dmr_counts_per_mirna_cancer, file = dmr_counts_file, 
            sep = "\t", quote = FALSE, row.names = FALSE)
cat(sprintf("\n✓ Saved DMR counts: %s\n", dmr_counts_file))

all_dmrs <- data.frame()

cancer_names <- names(all_dmrs_sig_list)
for (cancer in cancer_names) {
  dmrs_sig <- all_dmrs_sig_list[[cancer]]
  
  if (nrow(dmrs_sig) > 0) {
    dmr_info <- dmrs_sig[, c("DMR_ID", "Chromosome", "Start", "End", "miRNA", "DMR_Status")]
    dmr_info$Cancer <- cancer
    dmr_info$Coord <- paste(dmr_info$Chromosome, dmr_info$Start, dmr_info$End, sep = "_")
    all_dmrs <- rbind(all_dmrs, dmr_info)
  }
}

cat(sprintf("\nTotal significant DMRs across all cancers: %d\n\n", nrow(all_dmrs)))

cat("\nNORMALIZATION ANALYSIS\n")
cat("Formula: [Sum(Significant DMRs in all miRNAs)] / [Sum(Total DMRs in all miRNAs)] per cancer\n\n")

dmr_sig_counts_per_mirna_cancer <- all_dmrs %>%
  group_by(Cancer, miRNA, DMR_Status) %>%
  summarise(DMR_Count = n(), .groups = "drop")

total_dmrs_sig_per_mirna_cancer <- dmr_sig_counts_per_mirna_cancer %>%
  group_by(Cancer, miRNA) %>%
  summarise(Total_Sig_DMRs = sum(DMR_Count), .groups = "drop")

mirna_dmr_counts <- total_dmrs_sig_per_mirna_cancer %>%
  left_join(dmr_counts_per_mirna_cancer, by = c("Cancer", "miRNA"))

mirna_dmr_counts <- mirna_dmr_counts %>%
  filter(!is.na(Total_DMRs))

if (nrow(mirna_dmr_counts) == 0) {
  stop("No matching miRNA names between significant DMR data and total DMR counts!")
}

cat(sprintf(" Successfully matched %d miRNA-cancer pairs with DMR counts\n", 
            nrow(mirna_dmr_counts)))

normalization_per_cancer <- mirna_dmr_counts %>%
  group_by(Cancer) %>%
  summarise(
    Sum_Sig_DMRs = sum(Total_Sig_DMRs),
    Sum_Total_DMRs = sum(Total_DMRs),
    Normalization_Ratio = Sum_Sig_DMRs / Sum_Total_DMRs,
    .groups = "drop"
  )

cat("\nNormalization ratios per cancer:\n")
print(normalization_per_cancer)

dmr_status_with_total <- dmr_sig_counts_per_mirna_cancer %>%
  left_join(dmr_counts_per_mirna_cancer, by = c("Cancer", "miRNA")) %>%
  filter(!is.na(Total_DMRs))

dmr_status_sums <- dmr_status_with_total %>%
  group_by(Cancer, DMR_Status) %>%
  summarise(
    Sum_Sig_DMRs_Status = sum(DMR_Count),
    Sum_Total_DMRs = sum(Total_DMRs),
    .groups = "drop"
  )

dmr_status_sums <- dmr_status_sums %>%
  mutate(Normalized_Ratio = (Sum_Sig_DMRs_Status / Sum_Total_DMRs) * 100)

dmr_status_sums$Cancer <- factor(dmr_status_sums$Cancer, levels = cancer_types)

normalization_stats_file <- file.path(results_dir, "dmr_normalization_statistics.tsv")
write.table(dmr_status_sums, file = normalization_stats_file, 
            sep = "\t", quote = FALSE, row.names = FALSE)
cat(sprintf("\n✓ Saved normalization statistics: %s\n", normalization_stats_file))

p_status_norm <- ggplot(dmr_status_sums, aes(x = Cancer, y = Normalized_Ratio, fill = DMR_Status)) +
  geom_bar(stat = "identity", color = "black", position = position_dodge(width = 0.9)) +
  geom_text(aes(label = sprintf("%.2f%%", Normalized_Ratio)), 
            position = position_dodge(width = 0.9), 
            vjust = -0.5, size = 3) +
  scale_fill_manual(values = c("Hypermethylated" = "steelblue", "Hypomethylated" = "salmon")) +
  theme_minimal(base_size = 12) +
  labs(
    title = "Hypermethylated vs Hypomethylated miRNA Gene Body DMRs per Cancer Type",
    subtitle = "Normalized as: (Significant DMR count / Total DMR count) × 100",
    x = "Cancer Type", 
    y = "Normalized DMR Percentage (%)",
    fill = "DMR Status"
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
    plot.title = element_text(hjust = 0.5, face = "bold"),
    plot.subtitle = element_text(hjust = 0.5, size = 10),
    legend.position = "top"
  ) +
  ylim(0, max(dmr_status_sums$Normalized_Ratio) * 1.15)

plot_status_norm_file <- file.path(results_dir, "dmr_status_hyper_hypo_genebody_per_cancer_NORMALIZED.pdf")
ggsave(plot_status_norm_file, p_status_norm, width = 14, height = 6)
cat(sprintf("✓ Saved Plot 1: %s\n", plot_status_norm_file))

dmr_coords <- unique(all_dmrs$Coord)
dmr_concordance <- data.frame()

cat(sprintf("\nAnalyzing concordance across %d unique DMR positions...\n", length(dmr_coords)))

for (coord in dmr_coords) {
  coord_data <- all_dmrs[all_dmrs$Coord == coord, ]
  num_cancers <- length(unique(coord_data$Cancer))
  
  if (num_cancers >= 2) {
    all_hyper <- all(coord_data$DMR_Status == "Hypermethylated")
    all_hypo <- all(coord_data$DMR_Status == "Hypomethylated")
    
    if (all_hyper) {
      concordance <- "All_Hypermethylated"
    } else if (all_hypo) {
      concordance <- "All_Hypomethylated"
    } else {
      concordance <- "Discordant"
    }
    
    unique_mirnas <- unique(coord_data$miRNA)
    unique_mirnas <- unique_mirnas[unique_mirnas != "" & !is.na(unique_mirnas)]
    
    dmr_concordance <- rbind(dmr_concordance, data.frame(
      Coord = coord,
      DMR_ID = paste(unique(coord_data$DMR_ID), collapse = ";"),
      Chromosome = coord_data$Chromosome[1],
      Start = coord_data$Start[1],
      End = coord_data$End[1],
      miRNA = paste(sort(unique_mirnas), collapse = ";"),
      Num_Cancers = num_cancers,
      Cancers = paste(sort(unique(coord_data$Cancer)), collapse = ";"),
      Concordance = concordance,
      stringsAsFactors = FALSE
    ))
  }
}

concordant_dmrs <- dmr_concordance[dmr_concordance$Concordance != "Discordant", ]

cat(sprintf("\nTotal gene body DMR positions in 2+ cancers: %d\n", nrow(dmr_concordance)))
cat(sprintf("Concordant gene body DMRs (all same direction): %d\n", nrow(concordant_dmrs)))
cat(sprintf("  All Hypermethylated: %d\n", sum(concordant_dmrs$Concordance == "All_Hypermethylated")))
cat(sprintf("  All Hypomethylated: %d\n", sum(concordant_dmrs$Concordance == "All_Hypomethylated")))
cat(sprintf("Discordant gene body DMRs (mixed directions): %d\n\n", sum(dmr_concordance$Concordance == "Discordant")))

cat("Distribution of gene body DMRs by number of cancers:\n")
cancer_count_table <- table(dmr_concordance$Num_Cancers)
print(cancer_count_table)
cat("\n")

if (nrow(concordant_dmrs) > 0) {
  
  mirna_cancer_dmr <- list()
  
  for (i in 1:nrow(concordant_dmrs)) {
    miRNAs <- unlist(strsplit(concordant_dmrs$miRNA[i], ";"))
    miRNAs <- miRNAs[miRNAs != "" & !is.na(miRNAs)]
    miRNAs <- trimws(miRNAs)
    
    cancers <- unlist(strsplit(concordant_dmrs$Cancers[i], ";"))
    cancers <- trimws(cancers)
    
    dmr_coord <- concordant_dmrs$Coord[i]
    
    for (mirna in miRNAs) {
      for (cancer in cancers) {
        key <- paste(mirna, cancer, sep = "||")
        if (is.null(mirna_cancer_dmr[[key]])) {
          mirna_cancer_dmr[[key]] <- character()
        }
        mirna_cancer_dmr[[key]] <- c(mirna_cancer_dmr[[key]], dmr_coord)
      }
    }
  }
  
  all_mirnas <- unique(unlist(strsplit(concordant_dmrs$miRNA, ";")))
  all_mirnas <- all_mirnas[all_mirnas != "" & !is.na(all_mirnas)]
  all_mirnas <- trimws(all_mirnas)
  all_mirnas <- sort(all_mirnas)
  
  all_cancers_with_dmrs <- unique(unlist(strsplit(concordant_dmrs$Cancers, ";")))
  all_cancers_with_dmrs <- trimws(all_cancers_with_dmrs)
  all_cancers_with_dmrs <- sort(all_cancers_with_dmrs)
  
  dmr_matrix <- matrix(0, nrow = length(all_mirnas), ncol = length(all_cancers_with_dmrs),
                       dimnames = list(all_mirnas, all_cancers_with_dmrs))
  
  for (mirna in all_mirnas) {
    for (cancer in all_cancers_with_dmrs) {
      key <- paste(mirna, cancer, sep = "||")
      if (!is.null(mirna_cancer_dmr[[key]])) {
        dmr_matrix[mirna, cancer] <- length(unique(mirna_cancer_dmr[[key]]))
      }
    }
  }
  
  dmr_matrix_norm <- dmr_matrix
  
  for (mirna in rownames(dmr_matrix_norm)) {
    for (cancer in colnames(dmr_matrix_norm)) {
      dmr_count <- dmr_counts_per_mirna_cancer %>%
        filter(miRNA == mirna, Cancer == cancer) %>%
        pull(Total_DMRs)
      
      if (length(dmr_count) > 0 && dmr_count > 0) {
        dmr_matrix_norm[mirna, cancer] <- (dmr_matrix[mirna, cancer] / dmr_count) * 100
      } else {
        dmr_matrix_norm[mirna, cancer] <- 0
      }
    }
  }
  
  dmr_matrix_df <- as.data.frame(dmr_matrix)
  dmr_matrix_df <- cbind(miRNA = rownames(dmr_matrix_df), dmr_matrix_df)
  rownames(dmr_matrix_df) <- NULL
  
  dmr_matrix_norm_df <- as.data.frame(dmr_matrix_norm)
  dmr_matrix_norm_df <- cbind(miRNA = rownames(dmr_matrix_norm_df), dmr_matrix_norm_df)
  rownames(dmr_matrix_norm_df) <- NULL
  
  output_matrix_file <- file.path(results_dir, "miRNA_genebody_cancer_dmr_matrix.tsv")
  write.table(dmr_matrix_df, file = output_matrix_file, sep = "\t", quote = FALSE, row.names = FALSE)
  
  output_matrix_norm_file <- file.path(results_dir, "miRNA_genebody_cancer_dmr_matrix_NORMALIZED.tsv")
  write.table(dmr_matrix_norm_df, file = output_matrix_norm_file, sep = "\t", quote = FALSE, row.names = FALSE)
  
  cat(sprintf("\nmiRNA Gene Body x Cancer DMR matrix created with %d miRNAs and %d cancer types\n", 
              length(all_mirnas), length(all_cancers_with_dmrs)))
  cat(sprintf("Raw matrix saved to: %s\n", output_matrix_file))
  cat(sprintf("Normalized matrix saved to: %s\n\n", output_matrix_norm_file))
  
  cat("Matrix summary:\n")
  cat(sprintf("  Total miRNAs: %d\n", nrow(dmr_matrix)))
  cat(sprintf("  Total cancer types: %d\n", ncol(dmr_matrix)))
  cat(sprintf("  Total non-zero entries: %d\n", sum(dmr_matrix > 0)))
  cat(sprintf("  Max DMRs for any miRNA-cancer pair: %d\n", max(dmr_matrix)))
  
  mirna_totals <- rowSums(dmr_matrix)
  top_mirnas <- head(sort(mirna_totals, decreasing = TRUE), 20)
  cat("\nTop 20 miRNAs by total gene body DMRs across all cancers:\n")
  print(top_mirnas)
  
  miRNA_concordant_counts <- data.frame()
  
  for (i in 1:nrow(concordant_dmrs)) {
    miRNAs <- unlist(strsplit(concordant_dmrs$miRNA[i], ";"))
    miRNAs <- miRNAs[miRNAs != "" & !is.na(miRNAs)]
    miRNAs <- trimws(miRNAs)
    
    for (mirna in miRNAs) {
      concordance_type <- concordant_dmrs$Concordance[i]
      num_cancers <- concordant_dmrs$Num_Cancers[i]
      
      miRNA_concordant_counts <- rbind(miRNA_concordant_counts,
                                       data.frame(miRNA = mirna,
                                                  Concordance = concordance_type,
                                                  Num_Cancers = num_cancers,
                                                  stringsAsFactors = FALSE))
    }
  }
  
  miRNA_summary <- data.frame()
  for (mirna in unique(miRNA_concordant_counts$miRNA)) {
    mirna_data <- miRNA_concordant_counts[miRNA_concordant_counts$miRNA == mirna, ]
    
    total_concordant <- nrow(mirna_data)
    all_hyper <- sum(mirna_data$Concordance == "All_Hypermethylated")
    all_hypo <- sum(mirna_data$Concordance == "All_Hypomethylated")
    
    mirna_rows <- grepl(paste0("(^|;)", mirna, "(;|$)"), concordant_dmrs$miRNA)
    num_cancers <- length(unique(unlist(strsplit(
      paste(concordant_dmrs$Cancers[mirna_rows], collapse = ";"), ";"))))
    
    miRNA_summary <- rbind(miRNA_summary,
                           data.frame(miRNA = mirna,
                                      Total_Concordant_DMRs = total_concordant,
                                      All_Hypermethylated_DMRs = all_hyper,
                                      All_Hypomethylated_DMRs = all_hypo,
                                      Num_Cancers = num_cancers,
                                      stringsAsFactors = FALSE))
  }
  
  miRNA_summary <- miRNA_summary[order(-miRNA_summary$Total_Concordant_DMRs), ]
  
  output_file1 <- file.path(results_dir, "concordant_miRNA_genebody_DMRs_summary.tsv")
  write.table(miRNA_summary, file = output_file1, sep = "\t", quote = FALSE, row.names = FALSE)
  
  output_file2 <- file.path(results_dir, "concordant_genebody_DMRs_detailed.tsv")
  write.table(concordant_dmrs, file = output_file2, sep = "\t", quote = FALSE, row.names = FALSE)
  
  cat("\nAnalysis complete.\n")
  cat(sprintf("miRNA Gene Body-Cancer DMR matrix: %s\n", output_matrix_file))
  cat(sprintf("miRNA Gene Body-Cancer NORMALIZED DMR matrix: %s\n", output_matrix_norm_file))
  cat(sprintf("Concordant miRNA gene body DMRs summary: %s\n", output_file1))
  cat(sprintf("Concordant gene body DMRs detailed: %s\n", output_file2))
  
  cancer_count_df <- as.data.frame(table(dmr_concordance$Num_Cancers))
  colnames(cancer_count_df) <- c("Num_Cancers", "Count")
  cancer_count_df$Num_Cancers <- as.numeric(as.character(cancer_count_df$Num_Cancers))
  
  total_concordant_dmrs <- sum(cancer_count_df$Count)
  cancer_count_df$Normalized_Percent <- (cancer_count_df$Count / total_concordant_dmrs) * 100
  
  p2 <- ggplot(cancer_count_df, aes(x = Num_Cancers, y = Normalized_Percent)) +
    geom_bar(stat = "identity", fill = "steelblue", color = "black") +
    geom_text(aes(label = sprintf("%.1f%%", Normalized_Percent)), 
              vjust = -0.5, size = 3.5) +
    theme_minimal(base_size = 12) +
    labs(title = "Distribution of Gene Body DMRs by Number of Cancers",
         x = "Number of Cancers",
         y = "Percentage of Total Concordant DMRs (%)") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
    ylim(0, max(cancer_count_df$Normalized_Percent) * 1.15)
  
  plot2_file <- file.path(results_dir, "genebody_dmr_cancer_distribution_NORMALIZED.pdf")
  ggsave(plot2_file, p2, width = 10, height = 6)
  cat(sprintf(" Saved Plot 2: %s\n", plot2_file))
  
  top20_summary <- head(miRNA_summary, 20)
  
  top20_dmr_summary <- data.frame()
  for (mirna in top20_summary$miRNA) {
    dmr_data <- dmr_counts_per_mirna_cancer %>%
      filter(miRNA == mirna)
    
    if (nrow(dmr_data) > 0) {
      top20_dmr_summary <- rbind(top20_dmr_summary, data.frame(
        miRNA = mirna,
        Mean_DMRs = mean(dmr_data$Total_DMRs),
        stringsAsFactors = FALSE
      ))
    }
  }
  
  top20_summary <- top20_summary %>%
    left_join(top20_dmr_summary, by = "miRNA")
  
  top20_summary$Hyper_Percent <- (top20_summary$All_Hypermethylated_DMRs / top20_summary$Mean_DMRs) * 100
  top20_summary$Hypo_Percent <- (top20_summary$All_Hypomethylated_DMRs / top20_summary$Mean_DMRs) * 100
  
  top20_long <- top20_summary %>%
    select(miRNA, Hyper_Percent, Hypo_Percent, Mean_DMRs, Total_Concordant_DMRs) %>%
    pivot_longer(
      cols = c(Hyper_Percent, Hypo_Percent),
      names_to = "DMR_Status",
      values_to = "Percent"
    ) %>%
    mutate(
      DMR_Status = ifelse(DMR_Status == "Hyper_Percent", 
                          "Hypermethylated", 
                          "Hypomethylated")
    )
  
  top20_long$miRNA <- factor(top20_long$miRNA, levels = rev(top20_summary$miRNA))
  top20_long$DMR_Status <- factor(top20_long$DMR_Status, 
                                  levels = c("Hypomethylated", "Hypermethylated"))
  
  p3 <- ggplot(top20_long, aes(x = miRNA, y = Percent, fill = DMR_Status)) +
    geom_bar(stat = "identity", color = "black", position = "stack") +
    geom_text(aes(label = ifelse(Percent > 2, sprintf("%.1f%%", Percent), "")),
              position = position_stack(vjust = 0.5),
              size = 3, color = "white", fontface = "bold") +
    scale_fill_manual(values = c("Hypomethylated" = "salmon",
                                 "Hypermethylated" = "steelblue")) +
    coord_flip() +
    theme_minimal(base_size = 12) +
    labs(
      title = "Top 20 miRNAs by Concordant Gene Body DMR Count (Normalized by Total DMRs)",
      subtitle = "Normalized as: (Significant DMR count / Mean Total DMR count) × 100",
      x = "miRNA",
      y = "Normalized DMR Percentage (%)",
      fill = "DMR Status"
    ) +
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
      plot.subtitle = element_text(hjust = 0.5, size = 10),
      legend.position = "top",
      legend.title = element_text(face = "bold"),
      axis.text.y = element_text(size = 10)
    )
  
  plot3_file <- file.path(results_dir, "top20_mirnas_genebody_dmrs_NORMALIZED.pdf")
  ggsave(plot3_file, p3, width = 11, height = 8)
  cat(sprintf("Saved Plot 3: %s\n", plot3_file))
  
  chr_counts <- table(concordant_dmrs$Chromosome)
  chr_df <- data.frame(
    Chromosome = names(chr_counts),
    Count = as.numeric(chr_counts)
  )
  
  chr_df <- chr_df[!chr_df$Chromosome %in% c("chrX", "chrY", "chrM"), ]
  
  if (nrow(chr_df) > 0) {
    total_chr_dmrs <- sum(chr_df$Count)
    chr_df$Normalized_Percent <- (chr_df$Count / total_chr_dmrs) * 100
    
    chr_order <- paste0("chr", 1:22)
    chr_df$Chromosome <- factor(chr_df$Chromosome, 
                                levels = chr_order[chr_order %in% chr_df$Chromosome])
    chr_df <- chr_df[order(chr_df$Chromosome), ]
    
    p4 <- ggplot(chr_df, aes(x = Chromosome, y = Normalized_Percent)) +
      geom_bar(stat = "identity", fill = "mediumpurple", color = "black") +
      geom_text(aes(label = sprintf("%.1f%%", Normalized_Percent)), 
                vjust = -0.5, size = 3) +
      theme_minimal(base_size = 11) +
      labs(title = "Concordant Gene Body DMRs Distribution Across Chromosomes",
           x = "Chromosome",
           y = "Percentage of Total DMRs (%)") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1),
            plot.title = element_text(hjust = 0.5, face = "bold")) +
      ylim(0, max(chr_df$Normalized_Percent) * 1.15)
    
    plot4_file <- file.path(results_dir, "genebody_dmrs_by_chromosome_NORMALIZED.pdf")
    ggsave(plot4_file, p4, width = 12, height = 6)
    cat(sprintf(" Saved Plot 4: %s\n", plot4_file))
  }
  
  cancer_norm_data <- normalization_per_cancer %>%
    mutate(Normalized_Percent = Normalization_Ratio * 100) %>%
    arrange(desc(Normalized_Percent))
  
  cancer_norm_data$Cancer <- factor(cancer_norm_data$Cancer, levels = cancer_norm_data$Cancer)
  
  p5 <- ggplot(cancer_norm_data, aes(x = Cancer, y = Normalized_Percent)) +
    geom_bar(stat = "identity", fill = "salmon", color = "black") +
    geom_text(aes(label = sprintf("%.2f%%", Normalized_Percent)), 
              vjust = -0.5, size = 3, angle = 0) +
    theme_minimal(base_size = 11) +
    labs(
      title = "Concordant Gene Body DMRs per Cancer Type (Normalized by Total DMRs)",
      subtitle = "Normalized as: (Sum Significant DMRs / Sum Total DMRs) × 100",
      x = "Cancer Type",
      y = "Normalized DMR Percentage (%)"
    ) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
      plot.title = element_text(hjust = 0.5, face = "bold"),
      plot.subtitle = element_text(hjust = 0.5, size = 10)
    ) +
    ylim(0, max(cancer_norm_data$Normalized_Percent) * 1.15)
  
  plot5_file <- file.path(results_dir, "genebody_dmrs_per_cancer_NORMALIZED.pdf")
  ggsave(plot5_file, p5, width = 12, height = 6)
  cat(sprintf(" Saved Plot 5: %s\n", plot5_file))
  
  min_cancer_types <- 2
  top_n_mirnas <- 20
  
  if (exists("dmr_matrix_norm")) {
    cat("\nHEATMAP\n")
    cat(sprintf("Original matrix dimensions: %d miRNAs x %d cancers\n", 
                nrow(dmr_matrix_norm), ncol(dmr_matrix_norm)))
    cat(sprintf("Total non-zero entries: %d\n", sum(dmr_matrix_norm > 0)))
    
    multi_cancer_mirnas <- rowSums(dmr_matrix_norm > 0) >= min_cancer_types
    cat(sprintf("miRNAs present in >= %d cancers: %d\n", 
                min_cancer_types, sum(multi_cancer_mirnas)))
    
    dmr_matrix_filtered <- dmr_matrix_norm[multi_cancer_mirnas, , drop = FALSE]
    
    if (nrow(dmr_matrix_filtered) == 0) {
      cat("WARNING: No miRNAs found in >= 2 cancers. Lowering threshold to 1.\n")
      min_cancer_types <- 1
      multi_cancer_mirnas <- rowSums(dmr_matrix_norm > 0) >= min_cancer_types
      dmr_matrix_filtered <- dmr_matrix_norm[multi_cancer_mirnas, , drop = FALSE]
    }
  } else {
    stop("dmr_matrix_norm does not exist in environment!")
  }
  
  if (nrow(dmr_matrix_filtered) > top_n_mirnas) {
    mirna_sums <- rowSums(dmr_matrix_filtered)
    top_mirnas_idx <- order(mirna_sums, decreasing=TRUE)[1:top_n_mirnas]
    dmr_matrix_top <- dmr_matrix_filtered[top_mirnas_idx, , drop=FALSE]
    cat(sprintf("Selected top %d miRNAs by normalized DMR percentage\n", top_n_mirnas))
  } else {
    dmr_matrix_top <- dmr_matrix_filtered
    cat(sprintf("Using all %d filtered miRNAs (less than top_n threshold)\n", 
                nrow(dmr_matrix_top)))
  }
  
  dmr_matrix_top <- dmr_matrix_top[rowSums(dmr_matrix_top) > 0, , drop = FALSE]
  dmr_matrix_top <- dmr_matrix_top[, colSums(dmr_matrix_top) > 0, drop = FALSE]
  
  cat(sprintf("After removing zero rows/cols: %d rows, %d cols\n", 
              nrow(dmr_matrix_top), ncol(dmr_matrix_top)))
  
  if (nrow(dmr_matrix_top) == 0 || ncol(dmr_matrix_top) == 0) {
    cat("\n!!! ERROR: No data remaining for heatmap after filtering !!!\n")
    cat("Skipping heatmap generation.\n")
  } else {
    cat(sprintf("Matrix shape: %d miRNAs x %d cancers\n", 
                nrow(dmr_matrix_top), ncol(dmr_matrix_top)))
    cat(sprintf("Number of nonzero cells: %d\n", sum(dmr_matrix_top > 0)))
    cat(sprintf("Value range: %.2f%% to %.2f%%\n", min(dmr_matrix_top), max(dmr_matrix_top)))
    cat("\nMatrix preview (first 5 rows, first 5 cols):\n")
    print(dmr_matrix_top[1:min(5, nrow(dmr_matrix_top)), 
                         1:min(5, ncol(dmr_matrix_top))])
    
    plot6_file <- file.path(results_dir, "miRNA_genebody_cancer_heatmap_DMR_NORMALIZED.pdf")
    
    tryCatch({
      pdf(plot6_file, width=14, height=12)
      
      pheatmap(
        dmr_matrix_top,
        color = colorRampPalette(c("white", "yellow", "orange", "red", "darkred"))(100),
        cluster_rows = TRUE,
        cluster_cols = TRUE,
        show_rownames = TRUE,
        show_colnames = TRUE,
        fontsize_row = 8,
        fontsize_col = 10,
        main = paste0("Normalized Concordant Gene Body DMRs: Top ", nrow(dmr_matrix_top),
                      " miRNAs\n(Significant DMR / Total DMR × 100%)"),
        border_color = "grey60"
      )
      
      dev.off()
      cat(sprintf("Saved Plot 6 (Heatmap - normalized by total DMRs): %s\n", plot6_file))
      
    }, error = function(e) {
      dev.off()
      cat(sprintf("\nERROR generating normalized heatmap: %s\n", e$message))
    })
  }
  
} else {
  cat("No concordant gene body DMRs found.\n")
}

cat("1. dmr_status_hyper_hypo_genebody_per_cancer_NORMALIZED.pdf\n")
cat("2. genebody_dmr_cancer_distribution_NORMALIZED.pdf\n")
cat("3. top20_mirnas_genebody_dmrs_NORMALIZED.pdf\n")
cat("4. genebody_dmrs_by_chromosome_NORMALIZED.pdf\n")
cat("5. genebody_dmrs_per_cancer_NORMALIZED.pdf\n")
cat("6. miRNA_genebody_cancer_heatmap_DMR_NORMALIZED.pdf\n")
cat("\nNormalization formula: (Significant DMR count / Total DMR count) × 100\n")
cat("Per-cancer normalization: (Sum Significant DMRs / Sum Total DMRs) × 100\n")
cat("\nDone!\n")