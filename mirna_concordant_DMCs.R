...
read_all_miRNA_cpgs <- function(cancer_type, results_dir, mirna_genes) {
  ...
  cat(sprintf(" Loaded %d CpGs in miRNA gene bodies from %s\n", nrow(all_cpgs), cancer_type))
  return(all_cpgs)
}
...
cpg_counts_file <- file.path(results_dir, "miRNA_CpG_counts_per_cancer.tsv")
...
cat("\n NORMALIZATION \n")
cat("Formula: [Sum(DMCs in all miRNAs)] / [Sum(CpGs in all miRNAs)] per cancer\n\n")
...
p_status_norm <- ggplot(dmc_status_sums, aes(x = Cancer, y = Normalized_Ratio, fill = DMC_Status)) +
  ...
  labs(
    title = "Hypermethylated vs Hypomethylated miRNA Gene Body DMCs per Cancer Type",
...
plot_status_norm_file <- file.path(results_dir, "dmc_status_hyper_hypo_genebody_per_cancer_NORMALIZED.pdf")
...
if (nrow(concordant_dmcs) > 0) {
  ...
  output_matrix_file <- file.path(results_dir, "miRNA_genebody_cancer_exact_dmc_matrix_teliko.tsv")
  write.table(dmc_matrix_df, file = output_matrix_file, sep = "\t", quote = FALSE, row.names = FALSE)
  
  output_matrix_norm_file <- file.path(results_dir, "miRNA_genebody_cancer_exact_dmc_matrix_NORMALIZED.tsv")
  write.table(dmc_matrix_norm_df, file = output_matrix_norm_file, sep = "\t", quote = FALSE, row.names = FALSE)
  
  cat(sprintf("\nmiRNA Gene Body x Cancer DMC matrix created with %d miRNAs and %d cancer types\n", 
  ...
  cat("\nTop 20 miRNAs by total exact coordinate gene body DMCs across all cancers:\n")
  print(top_mirnas)
  ...
  output_file1 <- file.path(results_dir, "concordant_miRNA_genebody_exact_DMCs_summary_teliko.tsv")
  write.table(miRNA_summary, file = output_file1, sep = "\t", quote = FALSE, row.names = FALSE)
  
  output_file2 <- file.path(results_dir, "concordant_genebody_exact_DMCs_detailed_teliko.tsv")
  ...
  cat(sprintf("Concordant miRNA gene body exact DMCs summary: %s\n", output_file1))
  cat(sprintf("Concordant gene body exact DMCs detailed: %s\n", output_file2))
  ...
  p2 <- ggplot(cancer_count_df, aes(x = Num_Cancers, y = Normalized_Percent)) +
    ...
    geom_text(aes(label = sprintf("%.1f%%", Normalized_Percent)), 
              vjust = -0.5, size = 3.5) +
    theme_minimal(base_size = 12) +
    labs(title = "Distribution of Exact Coordinate Gene Body DMCs by Number of Cancers",
         x = "Number of Cancers",
         y = "Percentage of Total Concordant DMCs (%)") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
    ylim(0, max(cancer_count_df$Normalized_Percent) * 1.15)
  
  plot2_file <- file.path(results_dir, "exact_genebody_dmc_cancer_distribution_NORMALIZED.pdf")
  ...
  p3 <- ggplot(top20_long, aes(x = miRNA, y = Percent, fill = DMC_Status)) +
    ...
    theme_minimal(base_size = 12) +
    labs(
      title = "Top 20 miRNAs by Concordant Gene Body DMC Count (Normalized by CpG)",
  ...
  plot3_file <- file.path(results_dir, "exact_top20_mirnas_genebody_dmcs_NORMALIZED.pdf")
  ...
  if (nrow(chr_df) > 0) {
    ...
    p4 <- ggplot(chr_df, aes(x = Chromosome, y = Normalized_Percent)) +
      ...
      labs(title = "Exact Coordinate Concordant Gene Body DMCs Distribution Across Chromosomes",
           x = "Chromosome",
    ...
    plot4_file <- file.path(results_dir, "exact_genebody_dmcs_by_chromosome_NORMALIZED.pdf")
  ...
  p5 <- ggplot(cancer_norm_data, aes(x = Cancer, y = Normalized_Percent)) +
    ...
    theme_minimal(base_size = 11) +
    labs(
      title = "Concordant Gene Body DMCs per Cancer Type (Normalized by CpG)",
  ...
  plot5_file <- file.path(results_dir, "exact_genebody_dmcs_per_cancer_NORMALIZED.pdf")
  ...
  if (nrow(dmc_matrix_top) == 0 || ncol(dmc_matrix_top) == 0) {
    ...
    plot6_file <- file.path(results_dir, "miRNA_genebody_cancer_heatmap_DMC_NORMALIZED.pdf")
...
cat("1. dmc_status_hyper_hypo_genebody_per_cancer_NORMALIZED.pdf\n")
cat("2. exact_genebody_dmc_cancer_distribution_NORMALIZED.pdf\n")
cat("3. exact_top20_mirnas_genebody_dmcs_NORMALIZED.pdf\n")
cat("4. exact_genebody_dmcs_by_chromosome_NORMALIZED.pdf\n")
cat("5. exact_genebody_dmcs_per_cancer_NORMALIZED.pdf\n")
cat("6. miRNA_genebody_cancer_heatmap_DMC_NORMALIZED.pdf\n")
